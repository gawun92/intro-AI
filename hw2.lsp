...
; when the function is going through tree,
; if there is nothing inside, just passing it by empty set
; if TREE is atom, make it to list
; if TREE is list and the front element is atom, add an atom in
; the left side of returning  
; it is visiting left to right
(defun BFS (TREE)
  (cond 
    ((null TREE) nil)
    ((atom TREE) (list TREE)) 
...
; if there is nothing inside, just passing it by empty set
; if TREE is atom, make it to list
; else the function is recusively going through the tree,
; the deep elements are added from right branch to left branch.
; it is visiting right to left
(defun DFS (TREE)
  (cond 
    ((null TREE) nil) 
    ((atom TREE) (list TREE)) 
...
; when traversing the tree branch, it keeps decreasing the depth and it would stop and
; return something when depth is being less than 0
(defun DFID (TREE DEPTH)
  (cond 
    ((null TREE) nil) ;if there is no TREE, return nil
    ((< DEPTH 0) nil)
    (t (append (DFID TREE (- DEPTH 1)) (HELP-DFID TREE DEPTH)))
  )
) 

; DEPTH is less than 0 means that the running of function is done.
(defun HELP-DFID (TREE DEPTH)
  (cond 
    ((null TREE) nil) ;if there is no TREE, return nil
    ((< DEPTH 0) nil) 
    ((atom TREE) (list TREE)) ;if there is an atom left, return it as a list
    ((and (atom (car TREE)) (null (cdr TREE))) (HELP-DFID (car TREE) (- DEPTH 1)))
    (t (append (HELP-DFID (car TREE) (- DEPTH 1)) (HELP-DFID (cdr TREE) DEPTH)))
  )
)
...
(defun next-state (s m c)
  (let* ((b_M (- 3 (- (car s) m)))
         (b_C (- 3 (- (car (cdr s)) c))))
    (cond 
        ; START : invalid form and return nill
        ((< (- (car s) m) 0) nil)
        ((< (- (car (cdr s)) c) 0) nil)
        ((< b_C 0) nil)
        ((< b_M 0) nil)
        ((and (> b_C b_M) (> b_M 0)) nil)
        ((and (> (- (car (cdr s)) c) (- (car s) m)) (> (- (car s) m) 0)) nil)
        ; END : invalid form and return nill
        (t (list (list b_M b_C (not (third s)))))
    )
  )
)
...
(defun mult-dfs (states path)
  (cond 
    ((null states) nil)  ; states is empty, return nil
    ((mc-dfs (car states) path) (mc-dfs (car states) path)) ;first of state
