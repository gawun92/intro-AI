...
(defun at-most-one-color (n c k)
  (cond 
    ((= k c) (list (list (* k n) (* -1 (* k n)))))
    (t (at-most-one-color-helper1 (at-least-one-color n c k))) 
  )
)


(defun at-most-one-color-helper1 (x)
  (cond 
    ((< (length x) 2) nil) 
    (t (append (at-most-one-color-helper2 (* -1 (first x) ) (rest x)) 
               (at-most-one-color-helper1 (rest x))))
     
  )
)

(defun at-most-one-color-helper2(x y)
  (cond 
    ((null y) nil)
    (t (append (list (list x (* -1 (first y)))) 
               (at-most-one-color-helper2 x (rest y))))
  )
)
...
(defun generate-edge-clauses-helper (e c k)
  (cond 
    ...
    (t  
      (append 
        (list (list (* -1 (node2var (first e) c k)) (* -1 (node2var (second e) c k)))) 
